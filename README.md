...
❗Чтобы скачать, используйте `git clone --recursive`❗

## Полезная информация

Буду рад любым pull request-ам с полезными советами!

Файлы связанные с нашей платой находятся в `boards/nxp/fmuk66-v3`, мной уже изменены `default.cmake` и `init/rc.board_defaults` так чтобы автоматически собирался и автозапускался `nxpcup`, так что их вполне возможно не придётся больше трогать.

Сама программа `nxpcup` находится в `src/examples/nxpcup`, насколько я понимаю только её нам и надо менять.

Команда для сборки прошивки: `make nxp_fmuk66-v3_default`

Команда для сборки из загрузки прошивки: `make nxp_fmuk66-v3_default upload`

## Примечания
* Перед тем как собирать прошивку нужно установить toolchain командой `bash ./Tools/setup/ubuntu.sh`. Скорее всего если у Вас не Ubuntu 18.04 или Ubuntu 20.04 оно не установятся. Так же они устанавливают всяких хлам, так что советую запускать только в виртуалке.
* ~~В инструкции написано что нужно поправить файл `src/modules/commander/Commander.cpp`, но если это сделать как сказано в инструкции прошивка не соберётся. Насколько я понимаю это связано с тем что репозиторий обновлялся с тех пор как писалась инструкция, если у кого-то есть какие-то соображения на этот счёт пишите пожалуйста.~~ Я перешёл на branch `stable`, в нём commander.cpp меняется нормально, так что я его поменял.


## Полезные ссылки
* [Инструкции от NXP](https://nxp.gitbook.io/nxp-cup/)
* [Более общие, но и более качественные инструкции от PX4](https://dev.px4.io/v1.11_noredirect/en/)
